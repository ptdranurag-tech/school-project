    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <div className="bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl px-8 py-6 border border-purple-100/50 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-blue-200/20 to-indigo-200/20 rounded-full blur-3xl -mr-32 -mt-32"></div>
          <div className="relative z-10">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
              🎓 Principal Dashboard
            </h1>
            <p className="text-gray-600 mt-2 text-lg">
              Welcome back, <span className="font-semibold text-blue-700">{user?.name || "Principal"}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: 'Total Teachers', value: '-', icon: '👨‍🏫', gradient: 'from-blue-500 to-cyan-500', bgGradient: 'from-blue-50 to-cyan-50' },
            { title: 'Total Students', value: '-', icon: '🎓', gradient: 'from-indigo-500 to-purple-500', bgGradient: 'from-indigo-50 to-purple-50' },
            { title: 'School', value: school?.schoolName || "Loading...", icon: '🏫', gradient: 'from-purple-500 to-pink-500', bgGradient: 'from-purple-50 to-pink-50' },
          ].map((stat, index) => (
        ...
        <div className="bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl p-8 border border-purple-100/50 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { label: 'Manage Teachers', icon: '👨‍🏫', gradient: 'from-blue-500 to-cyan-500' },
                { label: 'View Students', icon: '🎓', gradient: 'from-indigo-500 to-purple-500' },
                { label: 'Attendance Reports', icon: '📊', gradient: 'from-purple-500 to-pink-500' },
                { label: 'Generate Reports', icon: '📈', gradient: 'from-blue-600 to-indigo-600' },
              ].map((action, index) => (
                <button
                  key={index}
...
export default PrincipalDashbaord